/*
                       * This header is generated by classdump-dyld 1.0
                       * on Monday, September 28, 2020 at 7:01:40 PM Mountain Standard Time
                       * Operating System: Version 14.0 (Build 18J386)
                       * Image Source: /System/Library/PrivateFrameworks/CMCapture.framework/CMCapture
                       * classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
                       */


#import <CMCapture/CMCapture-Structs.h>
@class NSMutableArray;

@interface BWVideoOrientationTimeMachine : NSObject {

	NSMutableArray* _cache;
	unsigned long long _capacity;
	opaqueCMFormatDescriptionRef _formatDescription;
	unsigned _metadataLocalID;
	OpaqueFigSimpleMutexRef _propertyMutex;

}
+(void)initialize;
-(id)debugDescription;
-(void)dealloc;
-(id)copyVideoOrientationSbufFromPTS:(SCD_Struct_BW36)arg1 toPTS:(SCD_Struct_BW36)arg2 referencePTS:(SCD_Struct_BW36)arg3 ;
-(id)_cachedItemsFromPTSSeconds:(double)arg1 toPTSSeconds:(double)arg2 ;
-(opaqueCMSampleBufferRef)_newSbufWithCachedItem:(id)arg1 trimAtStartPTS:(SCD_Struct_BW36)arg2 referencePTS:(SCD_Struct_BW36)arg3 ;
-(id)initWithCapacity:(unsigned long long)arg1 formatDescription:(opaqueCMFormatDescriptionRef)arg2 metadataLocalID:(unsigned)arg3 ;
-(void)addItemToCacheWithPTS:(SCD_Struct_BW36)arg1 exifOrientation:(short)arg2 ;
@end

